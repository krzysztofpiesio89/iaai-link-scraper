...
const createPrismaClient = () => {
    ...
    try {
        ...
        if (error.message.includes('@prisma/client did not initialize yet')) {
            console.error('❌ Prisma client not initialized. Running prisma generate...');
            console.log('💡 This will be handled automatically by the postinstall script');
            throw new Error('Prisma client needs to be generated. Run: npx prisma generate');
        }
        throw error;
    }
};
...
const testConnection = async () => {
    try {
        ...
        console.log(`📡 Connected to: ${dbUrl.replace(/\/\/([^:]+):([^@]+)@/, '//$1:***@')}`);
        return true;
    } catch (error) {
        console.error('❌ Database connection failed:', error.message);
        if (error.message.includes('Prisma client needs to be generated')) {
            console.log('💡 Please ensure Prisma client is generated by running:');
...
const showConnectionInfo = () => {
    ...
    if (dbUrl) {
        ...
        console.log(`🔧 Prisma Client Initialized: ${isClientInitialized ? 'Yes' : 'No'}`);
    } else {
        console.log('❌ No database URL configured');
    }
};
